...
template <typename T> string Serializer<T>::write(Temporal<T> const *temporal) {
  stringstream ss;
  if (TInstant<T> const *instant = reinterpret_cast<TInstant<T> const *>(temporal)) {
  ...
  } else if (TSequence<T> const *sequence = reinterpret_cast<TSequence<T> const *>(temporal)) {
...
template <> string Serializer<bool>::write(bool const &value) { return value ? "t" : "f"; }

template <> string Serializer<int>::write(int const &value) { return to_string(value); }
...
template <typename T> string Serializer<T>::write(TInstantSet<T> const *instant_set) {
  stringstream ss;
  ss << *instant_set;
  return ss.str();
}

template <typename T> string Serializer<T>::write(TSequence<T> const *sequence) {
  stringstream ss;
  ss << *sequence;
  return ss.str();
}

template <typename T> string Serializer<T>::write(TSequenceSet<T> const *sequence_set) {
  stringstream ss;
  ss << *sequence_set;
  return ss.str();
}
...
template <typename T> string Serializer<T>::write(TimestampSet const *timestamp_set) {
  stringstream ss;
  ss << *timestamp_set;
  return ss.str();
}
