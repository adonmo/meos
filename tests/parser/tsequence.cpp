...
TEMPLATE_TEST_CASE("TSequence are parsed", "[parser][temporal]", int, float) {
  SECTION("only one TSequence present") {
    SECTION("only one inst present") {
      Parser<TestType> r("[10@2012-11-01)");

      unique_ptr<TSequence<TestType>> tseq = r.parseNextTSequence();
      ...
      CHECK_THROWS(r.parseNextTSequence());
    }
  ...
  SECTION("multiple TSequences present") {
    ...
    unique_ptr<TSequence<TestType>> tseq2 = r.parseNextTSequence();
