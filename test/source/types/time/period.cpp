...
TEST_CASE("periods are validated and constructed properly", "[period]") {
  ...
  SECTION("all constructors work") {
    ...
    SECTION("constructors with bounds specified") {
      SECTION("normal constructor") {
        period = make_unique<Period>(unix_time_point(2019, 9, 8), unix_time_point(2019, 9, 10),
                                     false, true);
      }
      SECTION("two string constructor") {
        period
            = make_unique<Period>("2019-09-08 01:00:00+01", "2019-09-10 01:00:00+01", false, true);
      }
      SECTION("single string constructor") {
        period = make_unique<Period>("(2019-09-08 01:00:00+01, 2019-09-10 01:00:00+01]");
      }
  ...
  SECTION("clearly invalid") {
    ...
    REQUIRE_THROWS_AS((Period{lower, upper, lower_inc, upper_inc}), std::invalid_argument);
  }
  ...
  SECTION("edge case where lower == upper") {
    ...
    if (!should_be_valid) {
      REQUIRE_THROWS_AS((Period{t, t, lower_inc, upper_inc}), std::invalid_argument);
    } else {
...
TEST_CASE("Period timespan", "[period]") {
  auto lower_inc = GENERATE(true, false);
  auto upper_inc = GENERATE(true, false);
  auto left = GENERATE(take(4, random(unix_time(2012, 1, 1), unix_time(2020, 1, 1))));
  time_t const minute = 60 * 1000;
  time_t const year = 365 * 24 * 60 * 60 * 1000L;
  auto duration = std::chrono::milliseconds(GENERATE(take(4, random(minute, year))));
  auto left_tp = std::chrono::system_clock::from_time_t(left / 1000L);
  auto period = *make_unique<Period>(left_tp, left_tp + duration, lower_inc, upper_inc).get();
...
TEST_CASE("Period shift", "[period]") {
  auto lower_inc = GENERATE(true, false);
  auto upper_inc = GENERATE(true, false);
  auto period_in = *make_unique<Period>(unix_time_point(2012, 1, 1), unix_time_point(2012, 1, 7),
                                        lower_inc, upper_inc)
                        .get();
...
TEST_CASE("Period overlap", "[period]") {
  ...
  SECTION("clear overlap") {
    auto period_1 = *make_unique<Period>(unix_time_point(2012, 1, 1), unix_time_point(2012, 4, 1),
                                         lower_inc, upper_inc)
                         .get();
    auto period_2 = *make_unique<Period>(unix_time_point(2012, 2, 1), unix_time_point(2012, 5, 1),
                                         lower_inc, upper_inc)
                         .get();
    REQUIRE(period_1.overlap(period_2));
  }
  SECTION("clearly no overlap") {
    auto period_1 = *make_unique<Period>(unix_time_point(2012, 1, 1), unix_time_point(2012, 2, 1),
                                         lower_inc, upper_inc)
                         .get();
    auto period_2 = *make_unique<Period>(unix_time_point(2012, 4, 1), unix_time_point(2012, 5, 1),
                                         lower_inc, upper_inc)
                         .get();
    REQUIRE(!period_1.overlap(period_2));
  }
  SECTION("borderline overlap case when only both periods are inclusive") {
    auto period_1 = *make_unique<Period>(unix_time_point(2012, 1, 1), unix_time_point(2012, 3, 1),
                                         lower_inc, upper_inc)
                         .get();
    auto period_2 = *make_unique<Period>(unix_time_point(2012, 3, 1), unix_time_point(2012, 5, 1),
                                         lower_inc, upper_inc)
                         .get();
...
TEST_CASE("Period contains timestamp", "[period]") {
  ...
  SECTION("borderline contains timestamp only on inclusive end") {
    auto period = *make_unique<Period>(unix_time_point(2012, 1, 1), unix_time_point(2012, 4, 1),
                                       lower_inc, upper_inc)
                       .get();
    REQUIRE(period.lower_inc() == period.contains_timestamp(unix_time_point(2012, 1, 1)));
    REQUIRE(period.upper_inc() == period.contains_timestamp(unix_time_point(2012, 4, 1)));
  }
}
